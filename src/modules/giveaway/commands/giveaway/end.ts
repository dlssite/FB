...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (messageId) {
      const result = await GiveawayService.executeEnd(interaction.client, context.tenantId, messageId, interaction.user.id);
      if (!result.success) {
      ...
      const successMsg = Translator.t('giveaway', 'end_success', context.lang);
    ...
    const dropdown = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
      new StringSelectMenuBuilder()
        .setCustomId('giveaway_action_end')
        .setPlaceholder('Select an active giveaway to end...')
        .addOptions(
          giveaways.map(g => ({
    ...
    const response = ContainerService.create({
      title: '🎁 End Active Giveaway',
      description: 'Please select one of the active giveaways from the dropdown list below to end it and draw the winner(s).',
      components: [dropdown],
      footer: true,
      interaction
    });
