...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (targetUser.bot) {
      return replyV2(interaction, ContainerService.simple('❌ You cannot share your role with a bot.', { color: 'Red' }) as any);
    }

    const roleData = await BoosterRepository.getRole(context.tenantId, interaction.guild.id, interaction.user.id);
    if (!roleData) {
      return replyV2(interaction, ContainerService.simple('❌ You need to forge a custom role first using `/booster role`.', { color: 'Red' }) as any);
    }

    const discordRole = interaction.guild.roles.cache.get(roleData.roleId);
    if (!discordRole) {
      return replyV2(interaction, ContainerService.simple('❌ Your custom role appears to have been deleted. Please recreate it.', { color: 'Red' }) as any);
    }
    ...
    if (!newBuddy) {
      return replyV2(interaction, ContainerService.simple('❌ Could not find that user in the server.', { color: 'Red' }) as any);
    }
